...
class CrugeStoredUser extends CActiveRecord implements ICrugeStoredUser
{
    ...
    */
    ...
    public function validate($attributes = null, $clearErrors = true)
    {
        ...
        foreach ($this->getFields() as $f) {
            if ($f->validateField() == false) {
                ...
                foreach ($f->getErrors() as $a_error_field) {
                    $error_field.=implode("<br>", $a_error_field)."<br>";
                }
                Yii::app()->user->setFlash('error', $error_field);
        ...
        return array(
            ...
            ,
                'message' => CrugeTranslator::t(
                    'logon',
                    'La clave sólo puede tener dígitos, letras o los símbolos {symbols} con longitud entre {min} y {max} caracteres',
            ...
            ,
                ...
                'message' => CrugeTranslator::t('logon', 'Por favor, verifique que entiende y acepta los terminos y condiciones.'),
            ),
            array(
                'verifyCode',
                ...
                'message' => CrugeTranslator::t('logon', 'El código de verificación no puede ser nulo.'),
            ),
            /*array(
                'verifyCode',
                'captcha',
    ...
    public function validate_unique($att, $params)
    {
        ...
        if ($model != null) {
            $duptext = CrugeTranslator::t('logon', '\'attribute\' already in use', array('attribute' => ucfirst($att)));
            // $duptext = CrugeTranslator::t('logon', 'El \'attribute\' indicado ya esta en uso', array('attribute' => ucfirst($att)));
            if ($this->scenario == 'insert') {
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'deleteConfirmation' => ucfirst(CrugeTranslator::t('borrar')),
    ...
    public function search()
    {
        ...
        /**/ 
        if (!(Yii::app()->params['showInvitado'] && Yii::app()->user->isSuperAdmin)) {
            $criteria->condition = "iduser <> :invitado AND iduser <> :admin"; 
            $criteria->params = array(':invitado'=> 2, ':admin'=> 3,);
        }
