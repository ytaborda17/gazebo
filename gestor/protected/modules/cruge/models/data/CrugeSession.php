    ...
    */
    public static function create($iduser, $durationMins)
    {
        ...
        if ($model->ipaddress != "127.0.0.1") {
            $geoData = unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.$model->ipaddress)); // get Geolocation Data
            $model->geoLocStatus       = $geoData['geoplugin_status'];
            $model->city               = $geoData['geoplugin_city'];
            $model->region             = $geoData['geoplugin_region'];
            $model->countryName        = $geoData['geoplugin_countryName'];
            $model->countryCode        = $geoData['geoplugin_countryCode'];
            $model->latitude           = $geoData['geoplugin_latitude'];
            $model->longitude          = $geoData['geoplugin_longitude'];
        }
        return $model;
    }
