...
*/
    ...
    public function validateField()
    {
        ...
        if (($_val == "") && ($this->required != 0)) {
            ...
            $this->addError(
                $this->fieldname
                ,
                $this->longname . CrugeTranslator::t(" no puede ser nulo.")
            );
        }
        ...
        if (strlen($_val) > $this->maxlength) {
            ...
            $this->addError(
                $this->fieldname
                ,
                ucfirst(CrugeTranslator::t("el tamano maximo permitido es "))
        ...
        if ((trim($this->useregexp) != "") && (trim($this->getFieldValue()) != "")) {
            ...
            if (preg_match("/" . trim($this->useregexp) . "/", $this->getFieldValue())) {
                ...
                $this->addError(
                    $this->fieldname
                    ,
                    ucfirst(CrugeTranslator::t($this->useregexpmsg))
                );
            }
        }
    ...
    */
    ...
    public function attributeLabels()
    {
        return array(
            'idfield' => 'Idfield',
            'fieldname' => ucfirst(CrugeTranslator::t('Nombre interno')),
            'longname' => ucfirst(CrugeTranslator::t('Nombre publico')),
            'position' => ucfirst(CrugeTranslator::t('Posicion')),
            'required' => ucfirst(CrugeTranslator::t('Requerido')),
            'fieldtype' => ucfirst(CrugeTranslator::t('Tipo')),
            'fieldsize' => ucfirst(CrugeTranslator::t('Ancho caracteres')),
            'maxlength' => ucfirst(CrugeTranslator::t('Longitud maxima')),
            'showinreports' => ucfirst(CrugeTranslator::t('Ver en reportes')),
            'useregexp' => ucfirst(CrugeTranslator::t('Expresion regular')),
            'useregexpmsg' => ucfirst(CrugeTranslator::t('Mensaje de error')),
            'predetvalue' => ucfirst(CrugeTranslator::t('Valor predeterminado / Opciones de lista')),
        );
    }
